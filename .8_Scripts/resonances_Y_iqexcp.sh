...
output_filename="/home/gueee/printer_data/config/03_Resonances_Measurments/shaper_calibrate_y_iqexcp_${time_str}_${date_str}.png"
...
if grep -q "^shaper_type_yiqexcp" "$config_file"; then
  # Update existing entries
  sed -i "s/^shaper_type_yiqexcp.*/shaper_type_yiqexcp = '$recommended_shaper'/" "$config_file"
else
  # Add new entry with a newline
  echo -e "\nshaper_type_yiqexcp = '$recommended_shaper'" >> "$config_file"
fi

if grep -q "^shaper_freq_yiqexcp" "$config_file"; then
  # Update existing entries
  sed -i "s/^shaper_freq_yiqexcp.*/shaper_freq_yiqexcp = $recommended_freq/" "$config_file"
else
  # Add new entry with a newline
  echo "shaper_freq_yiqexcp = $recommended_freq" >> "$config_file"
fi
