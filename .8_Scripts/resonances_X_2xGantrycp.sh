...
output_filename="/home/gueee/printer_data/config/03_Resonances_Measurments/shaper_calibrate_x_2xGantrycp_${time_str}_${date_str}.png"
...
if grep -q "^shaper_type_x2xGantrycp" "$config_file"; then
  # Update existing entries
  sed -i "s/^shaper_type_x2xGantrycp.*/shaper_type_x2xGantrycp = '$recommended_shaper'/" "$config_file"
else
  # Add new entry with a newline
  echo -e "\nshaper_type_x2xGantrycp = '$recommended_shaper'" >> "$config_file"
fi

if grep -q "^shaper_freq_x2xGantrycp" "$config_file"; then
  # Update existing entries
  sed -i "s/^shaper_freq_x2xGantrycp.*/shaper_freq_x2xGantrycp = $recommended_freq/" "$config_file"
else
  # Add new entry with a newline
  echo "shaper_freq_x2xGantrycp = $recommended_freq" >> "$config_file"
fi
