...
output_filename="/home/gueee/printer_data/config/03_Resonances_Measurments/shaper_calibrate_x_t1_${time_str}_${date_str}.png"
...
if grep -q "^shaper_type_xt1" "$config_file"; then
  # Update existing entries
  sed -i "s/^shaper_type_xt1.*/shaper_type_xt1 = '$recommended_shaper'/" "$config_file"
else
  # Add new entry with a newline
  echo -e "\nshaper_type_xt1 = '$recommended_shaper'" >> "$config_file"
fi

if grep -q "^shaper_freq_xt1" "$config_file"; then
  # Update existing entries
  sed -i "s/^shaper_freq_xt1.*/shaper_freq_xt1 = $recommended_freq/" "$config_file"
else
  # Add new entry with a newline
  echo "shaper_freq_xt1 = $recommended_freq" >> "$config_file"
fi
