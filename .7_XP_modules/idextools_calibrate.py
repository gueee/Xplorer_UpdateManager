...
class PrinterProbeMultiAxis:
    ...
    def _probe(self, speed, axis, sense, max_distance):
        ...
        try:
            ...
            self.gcode.respond_info(f"DEBUG: Probe state before movement: {pre}")
        except Exception as e:
            pre = None
            self.gcode.respond_info(f"DEBUG: Error checking probe state: {e}")

        if pre:
            self.gcode.respond_info("DEBUG: Probe is already triggered - this is the error!")
            raise self.printer.command_error("Probe triggered prior to movement")
        
        self.gcode.respond_info(f"DEBUG: Probe not triggered, proceeding with movement")
...
class ProbeEndstopWrapper:
    ...
    def _get_steppers(self):
        if self.idex and self.axis == 'x':
            # For IDEX with carriage system, we need to get the correct steppers
            # based on the active extruder
            try:
                toolhead = self.printer.lookup_object('toolhead')
                kin = toolhead.get_kinematics()
                
                # Get all steppers and find the ones for X axis
                all_steppers = kin.get_steppers()
                x_steppers = []
                
                for stepper in all_steppers:
                    try:
                        stepper_name = stepper.get_name()
                        # For IDEX, we want stepper_x and stepper_x1 for X axis
                        if stepper_name in ['stepper_x', 'stepper_x1']:
                            # Check if this stepper is for the current axis
                            if stepper.is_active_axis(self.axis):
                                x_steppers.append(stepper)
                    except:
                        # If we can't get the name, check if it's an active X axis stepper
                        if stepper.is_active_axis(self.axis):
                            x_steppers.append(stepper)
                ...
                if x_steppers:
                    ...
                    # Fallback to default if no specific steppers found
                ...
                self.printer.lookup_object('gcode').respond_info(
                    f"Warning: IDEX stepper detection failed: {e}, using default"
                )
                return self.mcu_endstop.get_steppers()
        else:
            return self.mcu_endstop.get_steppers()
...
class PrinterProbeMultiAxis:
    ...
    def _probe(self, speed, axis, sense, max_distance):
        ...
        if pre:
            self.gcode.respond_info("DEBUG: Probe is already triggered - attempting to reset")
            
            # Try to lift the toolhead to release the probe
            try:
                current_pos = toolhead.get_position()
                lift_pos = list(current_pos)
                lift_pos[2] += 10.0  # Lift Z by 10mm
                toolhead.manual_move(lift_pos, 10.0)
                toolhead.wait_moves()
                
                # Check probe state again after lift
                pre_after_lift = self.mcu_probe[axis].query_endstop(toolhead.get_last_move_time())
                self.gcode.respond_info(f"DEBUG: Probe state after lift: {pre_after_lift}")
                
                if pre_after_lift:
                    # Still triggered, try moving away from sensor area
                    self.gcode.respond_info("DEBUG: Probe still triggered, moving away from sensor")
                    away_pos = list(current_pos)
                    away_pos[0] -= 20.0  # Move X away by 20mm
                    away_pos[1] -= 20.0  # Move Y away by 20mm
                    toolhead.manual_move(away_pos, 20.0)
                    toolhead.wait_moves()
                    
                    # Final check
                    pre_final = self.mcu_probe[axis].query_endstop(toolhead.get_last_move_time())
                    self.gcode.respond_info(f"DEBUG: Probe state after moving away: {pre_final}")
                    
                    if pre_final:
                        self.gcode.respond_info("ERROR: Probe remains triggered after reset attempts")
                        raise self.printer.command_error("Probe triggered prior to movement and reset failed")
            except Exception as reset_error:
                self.gcode.respond_info(f"DEBUG: Reset attempt failed: {reset_error}")
                raise self.printer.command_error("Probe triggered prior to movement")
